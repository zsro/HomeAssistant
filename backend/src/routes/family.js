...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    if (!user.familyId) {
      return res.status(404).json({ error: '您还没有加入家庭' });
    }
...
router.put('/', authenticateToken, async (req, res) => {
  try {
    ...
    if (family.createdBy !== user.id) {
      return res.status(403).json({ error: '只有家庭创建者可以修改家庭信息' });
    }
...
router.post('/join', authenticateToken, async (req, res) => {
  try {
    ...
    if (!familyCode) {
      return res.status(400).json({ error: '请提供家庭码' });
    }

    const family = await db.family.findByCode(familyCode);
    if (!family) {
      return res.status(404).json({ error: '家庭码不存在' });
    }

    // 检查是否已在家庭中
    if (user.familyId === family.id) {
      return res.status(400).json({ error: '您已经在该家庭中' });
    }
...
router.post('/leave', authenticateToken, async (req, res) => {
  try {
    ...
    if (!user.familyId) {
      return res.status(400).json({ error: '您不在任何家庭中' });
    }
    ...
    res.status(500).json({ error: '离开家庭失败' });
  }
});
  ...
  try {
    ...
    if (user.familyId) {
      ...
      if (oldFamily && oldFamily.createdBy === user.id) {
        ...
        return res.status(400).json({ 
          error: '您已创建家庭，无法创建新家庭。请先删除原家庭或转让家庭。' 
        });
      }
    }
